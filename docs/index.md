# Bienvenid@s

El presente espacio recopila teoría y ejercicios prácticos sobre temas relacionados con la implementación de redes y servicios utilizando, siempre que sea posible, software de código abierto. Los contenidos intentan cubrir los objetivos propuestos en el diseño curricular de la materia **Instalación, mantenimiento y reparación de redes informáticas** correspondiente al séptimo año de estudios de la escuela secundaria técnica, con orientación **Técnico en Informática Profesional y Personal**, en Buenos Aires, Argentina. 

## Sistema operativo utilizado en las prácticas
Las clases se dictarán en un laboratorio virtual local ubicado en el instituto. No obstante, es posible obtener las herramientas de software que se utilizarán a lo largo de la cursada, a fin de reproducir las prácticas en el hogar: 

* **Ubuntu Desktop 16.04.4 LTS** 
	* Torrent: [[32 bits]](http://releases.ubuntu.com/16.04/ubuntu-16.04.4-desktop-i386.iso.torrent) [[64 bits]](http://releases.ubuntu.com/16.04/ubuntu-16.04.4-desktop-amd64.iso.torrent) 
	* ISO: [[32 bits]](http://releases.ubuntu.unc.edu.ar/xenial/ubuntu-16.04.4-desktop-i386.iso) [[64 bits]](http://releases.ubuntu.unc.edu.ar/xenial/ubuntu-16.04.4-desktop-amd64.iso)
* **Ubuntu Server 16.04.4 LTS** 
	* Torrent: [[32 bits]](http://releases.ubuntu.com/16.04/ubuntu-16.04.4-server-i386.iso.torrent) [[64 bits]](http://releases.ubuntu.com/16.04/ubuntu-16.04.4-server-amd64.iso.torrent)
	* ISO: [[32 bits]](http://releases.ubuntu.unc.edu.ar/xenial/ubuntu-16.04.4-server-i386.iso) [[64 bits]](http://releases.ubuntu.unc.edu.ar/xenial/ubuntu-16.04.4-server-amd64.iso)

